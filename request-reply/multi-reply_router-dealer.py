...
def requester(backend_address, requester_id, stop, num_requests, interval=1):
    ...
    # Set high water mark to allow multiple messages in queue
        ...
        while not stop.is_set():
            ...
            while not final:
                ...
                if poller.poll(1000):
                    ...
                    print(f"Requester {requester_id} waiting for more responses...")
...
def replier(frontend_address, stop, num_requests, id, multipart):
    try:
        ...
        while not stop.is_set():
            ...
            client = message[0]
            contents = decoder.decode(message[1])
            print(f"Replier received: {message}")
            parts = randint(1, multipart)
